...
// Create a custom hook for managing notifications
const useNotifications = () => {
  const [notifications, setNotifications] = useState(() => {
    if (typeof window !== 'undefined') {
      const savedNotifications = localStorage.getItem('notifications');
      return savedNotifications ? JSON.parse(savedNotifications) : [
        { id: 1, message: "Water level low in Tank 2" },
        { id: 2, message: "Temperature spike in Lettuce Room 1" },
        { id: 3, message: "Humidity levels critical in Tomato Section" },
      ];
    }
    return [];
  });
  ...
  return { notifications, clearAllNotifications, removeNotification };
};
