...
def check_and_create_entity(entity_id, attributes, entity_type):
    ...
    # If the entity does not exist (404), create it
        ...
        for shelve_id in selected_shelve_ids:
            if shelve_id not in existing_shelve_ids:
                ...
                for sensor_type in sensor_types:
                    ...
                    # Retrieve status from sensorMaintenance
                    # cursor.execute("SELECT status FROM sensorMaintenance WHERE shelve_id = %s AND sensor_type = %s", (shelve_id, sensor_type))
                    # maintenance_status = cursor.fetchone()

                    # Map maintenance status to Sensors table status
                    # if maintenance_status:
                        # If status is 'Normal', set status to 1 (active); otherwise, set to 0 (inactive)
                        # sensor_status = 1 if maintenance_status[0] == 'Normal' else 0
                    # else:
                        # Default to active (1) if no status is found in sensorMaintenance
                        # sensor_status = 1
                    sensor_status = 1  
                    # Print for debugging
...
# Function to continuously generate data (this would be triggered in a separate thread)
